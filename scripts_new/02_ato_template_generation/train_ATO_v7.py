...
# 模板工件保存位置
OUTPUT_ROOT = PROJECT_ROOT / "output" / "ato_phase_results_v2"
...
def train_one_station_pair(station_pair: str, level_target_times: Dict[str, float], data_dir: Path) -> Dict:
    ...
    # ======= 🚀 进化版逻辑：B-Spline 函数化重构轨迹 =======
    v_norm_curves = np.vstack(v_norm_curves)
    # 1. 依然先获取一个初步的中值趋势作为目标
    raw_median = np.median(v_norm_curves, axis=0)
    
    # 2. 准备样条曲线的自变量 (0.0 到 1.0 的归一化时间)
    tau = np.linspace(0.0, 1.0, len(raw_median))
    
    # 3. 使用 B-Spline 进行拟合 (k=3 代表三次样条，保证加速度连续)
    # 我们减少控制点数量（例如取 15-20 个），起到自动滤波和提取“本质特征”的作用
    knots = np.linspace(0, 1, 15) # 15个控制点足以描述加减速特征
    spline = make_interp_spline(tau, raw_median, k=3)
    v_ref_t = spline(tau)
    
    # 4. 动力学边界强校正：确保开头和结尾速度绝对为 0
    v_ref_t = np.clip(v_ref_t, 0, None)
    v_ref_t[0] = 0.0
    v_ref_t[-1] = 0.0
    
    # 5. 再次使用 Savgol 做最后的物理细节微调
