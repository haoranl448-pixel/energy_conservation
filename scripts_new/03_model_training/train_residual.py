# -*- coding: utf-8 -*-
"""
train_residual_final.py (进度显示修复版)
"""
import os
import glob
import pickle
...
def read_data(station_pair):
    pattern_new = os.path.join(project_root, "data", "data_processed", f"results_{station_pair}*.xlsx")
...
# ================= 训练函数 =================
def train_residual(station_pair):
    out_dir = os.path.join(project_root, "output", "models", "nn_results_residual", station_pair)
    os.makedirs(out_dir, exist_ok=True)
    print(f"\n🚀 [物理融合] 开始训练: {station_pair}")

    # 1. 读取
    df = read_data(station_pair)
    ...
    mass = df['重量'].values
    raw_X = pd.DataFrame({'v':v, 'a':a, 'e_phy':df['energy_phy'], 'grad':grad, 'mass':mass}).fillna(0).values
    ...
    # phy_val = df.loc[idx, 'energy_phy'].values[SEQ_LEN-1:]
    # real_val = df.loc[idx, 'energy_real'].values[SEQ_LEN-1:]
    ...
    # real_val = real_val[:L_min]
    # phy_val = phy_val[:L_min]
    # res_pred = res_pred[:L_min]
    
    # final_pred = phy_val + res_pred
    # final_pred = np.maximum(final_pred, 0)
    
    # r2 = r2_score(real_val, final_pred)
    # print(f"   ★ 物理融合 R2: {r2:.4f}")

    # plt.figure(figsize=(10, 5))
    # plt.plot(np.cumsum(real_val), label='真实值', color='black', linewidth=2)
    # plt.plot(np.cumsum(phy_val), label='纯物理 (Physics Only)', color='gray', linestyle='--')
    # plt.plot(np.cumsum(final_pred), label='物理+AI修正 (Fusion)', color='red', linestyle='--')
    # plt.title(f"物理信息融合预测效果 ({station_pair}) R2={r2:.3f}", fontsize=12)
    # plt.xlabel("时间步")
    # plt.ylabel("累积能耗 (Wh)")
    # plt.legend()
    # plt.grid(True, alpha=0.3)
    # plt.tight_layout()
    # plt.savefig(f"{out_dir}/physics_fusion.png")
    # plt.close()
    # print(f"   📊 图表保存: {out_dir}/physics_fusion.png")
...
# 能耗数据处理 (仅保留真实值和AI修正值)
    ...
    plt.title(f"区间运行综合对标图 ({station_pair})\n[速度 v | 位移 s | 融合能耗 E]", fontsize=12)
    plt.tight_layout()
