...
# Savitzky-Golay 滤波，用于平滑生成速度曲线。
from scipy.signal import savgol_filter
# glob/pickle：查找数据文件、读取模板工件。
import glob
import pickle
# Path：统一处理路径。
from pathlib import Path
# Dict/List/Optional：类型标注，便于理解函数输入输出。
from typing import Dict, List, Optional
# r2_score：对比生成曲线和真实曲线时计算拟合指标。
from sklearn.metrics import r2_score # 确保在脚本顶部导
# numpy/pandas：数值计算和表格处理。
import numpy as np
import pandas as pd
# matplotlib：输出各等级曲线图；Agg 后端适合批处理保存图片。
import matplotlib
matplotlib.use("Agg")
...
# 3. 结果输出目录：生成曲线、图表、summary 都写到这里。
...
# 判断“贴着 80 km/h 平顶”的速度容忍。
NEAR_CAP_TOL_KMH = 0.02
# 连续贴限速点数量超过该值，就认为是平顶边界解。
...
def build_curve_with_params(lam: float, theta: float, direction: int,
    ...
    """给定 lambda/theta/direction，生成一条完整速度曲线。"""

    # lambda 同时缩放加速段和制动段时间。
    T_acc_new = lam * art["T_acc_ref"]
    T_br_new = lam * art["T_br_ref"]
    # 中段时间由目标总时间扣除加速段和制动段得到。
...
def evaluate_lambda_interval(lam: float, T_target: float, art: Dict, dt: float,
                             vmax_mps: Optional[float]) -> Optional[Dict]:
    """评估某个 lambda 下，通过 theta 调节可覆盖的距离范围。"""
...
def solve_exact_params(T_target: float, art: Dict, dt: float,
                       vmax_mps: Optional[float]) -> Optional[Dict]:
    """搜索 lambda/theta，使生成曲线尽量同时满足目标时间和目标距离。"""
    ...
    # 根据目标距离在 D0 的上方还是下方，决定 theta 的方向和比例。
    if L_target >= D0:
...
def load_source_dataframe_for_station(art: Dict, station_pair: str) -> pd.DataFrame:
    """加载某个区间用于真实对比的原始 cleaned 数据。"""

    # 优先使用模板工件中记录的 source_files。
