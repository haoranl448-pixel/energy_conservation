...
OUT_DIR  = Path(project_root, "output","optimization","opt_results1")
...
def run_one_segment(params_row):
    ...
    MODEL_PTH     = NN_RESULT_DIR / "best_model.pth"
    ...
    for t in t_list:
        rv   = minimize(lambda vp: objective([vp, t]),
...
def main():
    ...
    params['TIME_LOWER'] = pd.to_numeric(params['TIME_LOWER'], errors='coerce')
    params['TIME_UPPER'] = pd.to_numeric(params['TIME_UPPER'], errors='coerce')
