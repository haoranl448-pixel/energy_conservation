...
当前推荐从 `scripts_new/main.py` 进入完整流程。各目录中的 `README.md` 已按统一口径列出每个代码文件的用途和状态：

整体业务流程图：`../docs/项目整体流程.drawio`（可编辑源文件）和 `../docs/project_flow_preview.png`（快速预览）。

| 目录 | 内容 | 当前定位 |
| --- | --- | --- |
| `00_main_pipeline/` | 数据处理、残差模型、ATO 曲线、能耗菜单、DP 规划 | 当前主流程 |
| `01_data_processing/` | 原始数据清洗、分段、等级和载重处理 | 数据处理历史工具 |
| `02_ato_template_generation/` | ATO 模板及速度曲线生成的历代实验 | 模型迭代记录 |
| `03_model_training/` | MLP、Transformer 和残差模型训练 | 训练实验；主线版本在 `00` |
| `04_energy_calculation/` | 单区间、全线和不同版本的能耗计算 | 计算实验 |
| `05_optimization_scheduling/` | 早期全局排图和专项区间优化 | 优化实验；主线 DP 在 `00` |
| `06_validation_reports/` | 历史与规划结果验证、旧版趟次报告 | 验证工具 |
| `07_visualization/` | 当前 Simulink 对比图及其他科研图表 | 当前绘图与历史绘图工具 |
| `08_analysis/` | 样本、载重、等级和误差统计 | 当前分析工具 |
| `09_exports_opentrack/` | timetable、OTD、限速和 OpenTrack 对比 | OpenTrack 接口工具 |
| `99_misc_legacy/` | 无法归入主流程的早期试验文件 | 仅供追溯 |

状态说明：

- **主线**：由 `scripts_new/main.py` 或当前批处理直接调用。
- **当前工具**：仍用于现阶段分析、绘图或 OpenTrack 操作，但不属于一键主流程。
- **历史/实验**：保留用于复现实验，不建议作为新任务入口。
- **停用**：代码已整体注释或仅保留文字记录，不能直接执行。

本次整理只增加索引和说明，不移动、不删除、不重命名业务脚本，避免影响硬编码路径和已有实验结果。

`scripts_new/` 根目录文件：

| 文件 | 具体作用 |
| --- | --- |
| `main.py` | 当前一键主程序，负责选择步骤、传递趟次/数据目录/追溯表参数、清理旧输出并记录分步日志。 |
| `_manifest.csv` | 从原 `scripts/` 复制和分类到 `scripts_new/` 时形成的文件清单，用于追溯原路径。 |

底层模块和测试另见：

- `../src/README.md`：物理模型、神经网络定义和公共封装。
- `../tests/README.md`：当前自动化回归测试。
